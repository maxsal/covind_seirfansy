...
date_31_jan = which(data$date == "2021-01-31")
data_train = data[date_1_April:date_31_jan, ]
obsP_tr <- data_train[,"Daily.Confirmed"] ## Daily Positive
obsR_tr <- data_train[,"Daily.Recovered"] ## Daily Recovered
obsD_tr <- data_train[,"Daily.Deceased"] ##  Daily Deaths
...
phases = c(1,15,34,48,62,92,123, 154, 184, 215, 245, 276)
...
P_pred=prediction[356*3+1:356,]
P_pred=rowMeans(P_pred)
R_pred=prediction[356*6+1:356,]
R_pred=rowMeans(R_pred)
D_pred=prediction[356*8+1:356,]
D_pred=rowMeans(D_pred)
T_d=R_pred+D_pred+P_pred
total_pred = rowSums(matrix(rowMeans(prediction), nrow = 356)[, 3:9])
UF_p =total_pred/T_d
UF_p[306]
total_pred[306]
D_U=prediction[356*7+1:356,]
D_U=rowMeans(D_U)
...
impo=data.frame("UnderReporting_Cases" = UF_p[306], "UnderReporting_Deaths" = UF_d[306], "ifr" = ifr[306])
